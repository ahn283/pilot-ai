...
3. **코드리뷰** — `senior-code-reviewer` 에이전트로 변경 코드 리뷰. 발견된 CRITICAL/HIGH 이슈는 반드시 수정.
4. **단위 테스트** — `npm test` 작성 및 통과 확인.
5. **체크리스트 업데이트** — `docs/checklist.md`에서 완료 항목 체크 (`- [x]`).
6. **커밋** — 위 1-5 모두 통과 후에만 커밋.
...
- 빌드 실패, 코드리뷰 CRITICAL/HIGH 미해결, 테스트 미작성/실패, 체크리스트 미반영 상태에서 절대 커밋하지 않는다.
