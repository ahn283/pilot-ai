...
describe('pull requests', () => {
  ...
  it('lists PRs', async () => {
    mockExecuteShell.mockResolvedValue({ exitCode: 0, stdout: '[{"number":1,"title":"test"}]', stderr: '' });
    const result = await listPrs();
    expect(result).toContain('"number":1');
  });

  it('gets PR details', async () => {
    mockExecuteShell.mockResolvedValue({ exitCode: 0, stdout: '{"number":5,"title":"PR"}', stderr: '' });
    const result = await getPr('5');
    expect(result).toContain('"number":5');
  });

  it('merges a PR', async () => {
    mockExecuteShell.mockResolvedValue({ exitCode: 0, stdout: 'Merged', stderr: '' });
  ...
  it('gets PR diff', async () => {
    mockExecuteShell.mockResolvedValue({ exitCode: 0, stdout: '+added line', stderr: '' });
...
describe('issues', () => {
  it('creates an issue', async () => {
    mockExecuteShell.mockResolvedValue({ exitCode: 0, stdout: 'https://github.com/repo/issues/10', stderr: '' });
    const url = await createIssue({ title: 'Bug', labels: ['bug'] });
    expect(url).toContain('issues/10');
  });

  it('lists issues', async () => {
    mockExecuteShell.mockResolvedValue({ exitCode: 0, stdout: '[{"number":1}]', stderr: '' });
    const result = await listIssues({ state: 'open' });
    expect(result).toContain('"number":1');
  });

  it('closes an issue', async () => {
    mockExecuteShell.mockResolvedValue({ exitCode: 0, stdout: 'Closed', stderr: '' });
...
describe('CI checks', () => {
  it('gets checks for HEAD', async () => {
    mockExecuteShell.mockResolvedValue({ exitCode: 0, stdout: '[{"name":"test","status":"completed"}]', stderr: '' });
    const result = await getChecks();
    expect(result).toContain('"name":"test"');
  });

  it('gets failed run log', async () => {
    mockExecuteShell.mockResolvedValue({ exitCode: 0, stdout: 'error log here', stderr: '' });
