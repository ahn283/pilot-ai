...
### 5.12 Webhook / HTTP API Endpoint (Phase 2)

로컬 HTTP 서버를 노출하여 외부에서 pilot-ai에 명령을 전달할 수 있게 한다.

```
POST http://localhost:3847/api/command
Authorization: Bearer {local-token}
Content-Type: application/json

{
  "command": "api 프로젝트에서 npm test 실행해줘",
  "project": "api"
}
```

**활용 시나리오:**
- **Apple Shortcuts**: "Hey Siri, pilot에게 배포해달라고 해" → Shortcut이 POST 요청
- **Raycast / Alfred**: 키보드 런처에서 빠른 명령 전달
- **GitHub Actions**: CI 완료 후 webhook으로 알림/후속 작업 트리거
- **cron**: 시스템 crontab에서 주기적 명령 전달

**보안:**
- localhost 전용 (외부 네트워크 노출 안 함)
- Bearer token 인증 (온보딩 시 랜덤 생성, Keychain 저장)
- Rate limiting

### 5.13 GitHub Integration (Phase 2)

기존 shell 도구와 `gh` CLI를 활용하여 GitHub 작업을 자연어로 수행한다.

```
사용자: "api 프로젝트에서 현재 브랜치로 PR 만들어줘"
사용자: "내 PR에 CI 통과했는지 확인해줘"
사용자: "이슈 #42를 fix로 닫아줘"
```

별도 API 클라이언트 없이 `gh pr create`, `gh issue list` 등 CLI 활용으로 구현 복잡도 낮음.

### 5.14 Clipboard / Screenshot (Phase 2)

macOS 내장 명령어를 활용한 클립보드/스크린샷 연동.

```
사용자: "클립보드에 있는 코드 분석해줘"      → pbpaste 실행 후 Claude에 전달
사용자: "화면 캡처해서 에러 분석해줘"        → screencapture 실행 후 Vision API
사용자: "이거 복사해둬: ..."                 → pbcopy로 클립보드에 저장
```

- `pbpaste` / `pbcopy`: macOS 내장, 추가 의존성 없음
- `screencapture -x /tmp/screen.png`: 무음 스크린샷
- Claude Vision API로 이미지 분석

### 5.15 Multi-modal Input (Phase 2)

Slack/Telegram에서 이미지 첨부 시 Claude Vision으로 분석.

```
사용자: [에러 스크린샷 첨부] "이 에러 고쳐줘"
사용자: [다이어그램 첨부] "이 아키텍처 설명해줘"
```

- Slack: `files:read` 권한으로 이미지 다운로드
- Telegram: `getFile` API로 이미지 다운로드
- Base64 인코딩 → Claude Vision API 또는 `claude -p` 파이프

### 5.16 macOS Notifications (Phase 2)

긴 작업 완료 시 macOS 네이티브 알림 전송.

```bash
osascript -e 'display notification "빌드 완료!" with title "Pilot AI"'
```

- 메신저 알림과 별도로 데스크톱 알림 제공
- 포커스 모드에서도 확인 가능
- `terminal-notifier` 사용 시 클릭 액션 연동 가능

### 5.17 Obsidian Integration (Phase 2)

Obsidian vault는 로컬 마크다운 파일이므로 기존 filesystem 도구로 직접 접근 가능.

```
사용자: "오늘 데일리 노트에 standup 내용 추가해줘"
사용자: "프로젝트 노트에서 migration 관련 내용 찾아줘"
```

- Obsidian vault 경로를 config에 등록
- 마크다운 파일 직접 읽기/쓰기 (별도 API 불필요)
- Daily note 템플릿 지원

### 5.18 Linear / Jira Integration (Phase 2)

이슈 트래커를 자연어로 관리.

```
사용자: "Linear에 이슈 만들어줘: 로그인 버그, priority high"
사용자: "내 이슈 목록 보여줘"
사용자: "이슈 FE-42 상태를 In Progress로 바꿔줘"
```

- Linear GraphQL API / Jira REST API
- 온보딩에서 API 키 입력 + Keychain 저장

### 5.19 Email Integration (Phase 3)

Gmail / Outlook 연동으로 이메일 관리.

```
사용자: "읽지 않은 이메일 요약해줘"
사용자: "X에게 답장 초안 작성해줘"
사용자: "[Dangerous] 이 답장 보내줘"
```

- Gmail API (OAuth2) 또는 Microsoft Graph API
- OAuth2 토큰 refresh 자동 관리
- 이메일 전송은 Dangerous 분류 (승인 필요)

### 5.20 Calendar Management (Phase 3)

일정 조회/생성/수정.

```
사용자: "내일 일정 알려줘"
사용자: "오후 2시에 2시간 집중 시간 블록 만들어줘"
사용자: "배포 직전에 미팅 없는지 확인해줘"
```

- Google Calendar API (OAuth2) 또는 Apple Calendar (EventKit via AppleScript)
- 시간대(timezone) 자동 처리
- 에이전트가 시간 맥락을 인지 ("미팅 전에 배포하지 않기")

### 5.21 Voice Input/Output (Phase 3)

음성 명령 및 응답.

```
[음성] "pilot, api 테스트 돌려줘"
[TTS] "8개 테스트 모두 통과했습니다."
```

- **TTS**: macOS `say` 명령 (추가 설치 불필요) 또는 ElevenLabs API
- **STT**: Whisper API 또는 Apple Speech (push-to-talk)
- Apple Shortcuts의 "Dictate Text" → Webhook으로 전달 가능

### 5.22 Multi-Agent Orchestration (Phase 3)

복잡한 작업을 역할별 서브 에이전트로 분업.

```
사용자: "새 기능 리서치하고 구현하고 테스트까지 해줘"

내부:
  1. Research Agent → 관련 문서/코드 조사
  2. Planning Agent → 구현 계획 수립
  3. Coding Agent → 코드 작성
  4. Review Agent → 코드 리뷰 + 테스트
```

- CrewAI/AutoGPT 패턴 참고
- 공유 컨텍스트 관리, 에러 복구
...
- **Webhook / HTTP API Endpoint** (외부 트리거, Apple Shortcuts 연동)
- **GitHub Integration** (`gh` CLI 연동, PR/이슈 관리)
- **Clipboard / Screenshot** (pbpaste, screencapture, 비전 분석)
- **Multi-modal Input** (이미지 첨부 → Claude Vision)
- **macOS Notifications** (osascript 알림 전송)
- **Obsidian Integration** (마크다운 기반 노트 연동)
- **Linear / Jira Integration** (이슈 트래커 자연어 관리)
...
- **Email Integration** (Gmail/Outlook, OAuth2 연동)
- **Calendar Management** (Google Calendar / Apple Calendar)
- **Voice Input/Output** (Whisper STT + macOS `say` TTS)
- **Multi-Agent Orchestration** (역할 기반 서브 에이전트 분업)
